...
const StoreContextProvider = (props) => {
  ...
  const handleChangeAnswer = (e) => {
    ...
    if (
      quizes[activeOrder].selects.type === "single-select" ||
      quizes[activeOrder].selects.type === "single-select-image"
    ) {
      const newAnswer = {
        order: activeOrder,
      ...
      setTimeout(function () {
        setIsLoader(false);
        if (activeOrder === 1) {
          const lang = quizes[activeOrder].selects.items.find(
            (item) => t(item.caption) === inputValue
          ).lang;
          i18n.changeLanguage(lang);
        }
      }, 300);
    } else if (
      quizes[activeOrder].selects.type === "multi-select" ||
      quizes[activeOrder].selects.type === "bubble"
    ) {
      if (e.target.checked) {
        setMultiAnsw([...multiAnsw, inputValue]);
      } else {
        setMultiAnsw((prevAnsw) =>
          prevAnsw.filter((answr) => answr !== inputValue)
        );
      }
    }
  };
  ...
  const isValidEmail = (email) => {
    let patternEmail =
      /^(([a-zA-Z0-9&+-=_])+((\.([a-zA-Z0-9&+-=_]){1,})+)?){1,64}@([a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\.)+[a-zA-Z]{2,6}$/;
  ...
  const handleBackToLevel = () => {
    setAnswers((prevAnswers) =>
      prevAnswers.filter((answ) => answ.order !== activeOrder - 1)
    );
    setActiveOrder(activeOrder - 1);
  };
